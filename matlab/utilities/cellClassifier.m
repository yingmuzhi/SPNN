classdef cellClassifier < handle
    ...
    methods (Access= public)
        ...
        function train(self, trainingFolder, trainingOptions)
            ...
            % 检查图片和mask数量是否一致
            if length(imList) ~= length(maskList)
                error('图片和mask数量不一致: %d vs %d (文件夹: %s)', length(imList), length(maskList), trainingFolder);
            end
            ...
            if trainingOptions.parallelSubset == 1
                ...
                parfor j = 1:length(imgSubset)
                    ...
                    for i = 1:length(imSub)
                        fprintf("[Parallel] Processing image %d/%d in subset %d\n", i, length(imSub), j);
                        fprintf("  Image file: %s\n", imSub{i});
                        fprintf("  Mask file:  %s\n", maskSub{i});
                        im = imread(imSub{i});
