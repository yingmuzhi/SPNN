...
%                 end
        ...
        function [T, meanSliceFluo] = quantifyDiffuseSimple(self, annotationVolume, channelNumber, varargin)
            % [T, meanSliceFluo] = quantifyDiffuseSimple(annotationVolume, channelNumber, varargin)
            % Simplified version of quantifyDiffuse that works without displacement fields and annotation data
            
            % Parse optional arguments
            p = inputParser;
            addParameter(p, 'skipDisplacementFields', false, @islogical);
            addParameter(p, 'skipAnnotationVolume', false, @islogical);
            addParameter(p, 'useManualRegionID', false, @islogical);
            addParameter(p, 'manualRegionID', 0, @isnumeric);
            parse(p, varargin{:});
            
            skipDisplacementFields = p.Results.skipDisplacementFields;
            skipAnnotationVolume = p.Results.skipAnnotationVolume;
            useManualRegionID = p.Results.useManualRegionID;
            manualRegionID = p.Results.manualRegionID;
            ...
            % Handle region data
            if ~skipAnnotationVolume && ~isempty(annotationVolume) && ~useManualRegionID
                try
                    % Prepare the annotation image
                    obliqueSlice = self.getObliqueSlice(annotationVolume);
                    if ~skipDisplacementFields && ~isempty(self.dispField)
                        annot = imwarp(obliqueSlice, self.dispField,'nearest');
                    else
                        annot = obliqueSlice;
                    end
                    ...
                    fprintf('Warning: Could not load annotation data: %s\n', ME.message);
                    fprintf('Using manual regionID instead...\n');
                    useManualRegionID = true;
                end
            end
            
            % If using manual regionID or annotation failed
            if useManualRegionID || skipAnnotationVolume || isempty(annotationVolume)
                regionIDs = manualRegionID;
                % Create a simple annotation map (all pixels in mask get the same regionID)
                annot = ones(size(raw)) * manualRegionID;
                annot = annot .* msk;
            end
            ...
            fprintf('SUMMARY\n')
            fprintf([repmat('*',1,30),'\n'])
            fprintf('\t%u regions quantified.\n',length(regionIDs))
            fprintf('\tTotal area: %u pixels\n',sum(areaPx))
            fprintf('\tTotal fluorescence: %.2f\n',sum(diffFluo))
            if skipDisplacementFields
                fprintf('\tDisplacement fields skipped.\n');
            end
            if skipAnnotationVolume || useManualRegionID
                fprintf('\tUsing simplified region analysis.\n');
            end
            fprintf([repmat('*',1,30),'\n'])
        end
          
        function T = quantifyDotsSimple(self, annotationVolume, channelNumber, rfModelPath, varargin)
            % T = quantifyDotsSimple(annotationVolume, channelNumber, rfModelPath, varargin)
            % Simplified version of quantifyDots that works without displacement fields and annotation data
            ...
            for i = 1:size(dotsT,1)
                ...
                % Add optional fields if data is available
                if ~skipAnnotationVolume && ~isempty(annotationVolume) || useManualRegionID
                    try
                        % Handle regionID
                        if useManualRegionID
                            % Use manually input regionID
                            T(count).regionID = manualRegionID;
                        else
                            % Prepare the annotation image
                            obliqueSlice = self.getObliqueSlice(annotationVolume);
                            if ~skipDisplacementFields && ~isempty(self.dispField)
                        ...
                        % Transformation matrix to get 3D positions
                        if ~isempty(self.u) && ~isempty(self.v) && ~isempty(self.o)
                        ...
                        fprintf('Warning: Could not process spatial data for cell %d: %s\n', i, ME.message);
                        T(count).regionID = NaN;
                        T(count).xCCF = NaN;
                        T(count).yCCF = NaN;
                        T(count).zCCF = NaN;
                    end
            ...
            if skipAnnotationVolume
                fprintf('\tAnnotation volume skipped.\n');
            end
            if useManualRegionID
                fprintf('\tUsing manual regionID: %d\n', manualRegionID);
            end
            fprintf([repmat('*',1,30),'\n'])
