...
% --- 脚本将根据以上配置自动构建路径 ---
rawTiffPath = fullfile(base_path, mouse, 'rawData');
flipInfoPath  = fullfile(base_path, mouse, 'flipInfo');
datasetPath = fullfile(base_path, mouse, 'DATASET');

% 检查并按需创建目录
if ~isfolder(flipInfoPath), mkdir(flipInfoPath); end
if ~isfolder(datasetPath), mkdir(datasetPath); end
...
if isfolder(rawTiffPath) == false
    error('指定的原始图像路径不存在或不是一个文件夹: %s', rawTiffPath);
end

flipInfoFile = fullfile(flipInfoPath, [mouse,'.xlsx']);
if isfile(flipInfoFile) == false
    fprintf('未找到翻转信息文件，正在创建: %s\n', [mouse '.xlsx']);

    infoTable = table(string(imgsN'), strings(size(imgsN, 2),1), 'VariableNames',{'Image', 'morePosterior'});
    writetable(infoTable,  flipInfoFile)
    fprintf('文件已创建。请在该文件中填写翻转信息 (l/r)，然后重新运行此脚本。\n');
    return;
else
    fprintf('找到并加载已存在的翻转信息文件: %s\n', [mouse '.xlsx']);
end
...
parfor i = 1:size(imgsP,2)
    ...
    imwrite(uint8(im), [targetFolder filesep imgsN{i}]);
end
...
%             255,prctile(smallIm(:,:,2),99.8,'all'),255];
    ...
    imwrite(smallIm, fullName, 'BitDepth', 8);
